# i18n (Mongo-backed) + Admin UI + AI translation + Audit + Encrypted settings (saasbackend)

## Goal
Provide a reusable i18n mini-system inside `saasbackend` that host apps (e.g. `ref-bauges`, `ref-enbauges`) can use in **middleware mode** and standalone mode.
...
- All mutations are auditable via a generic saasbackend audit system.
...
- Use AES-256-GCM.
- Encryption key comes from env: `SAASBACKEND_ENCRYPTION_KEY`.
...
- Call `saasbackend.i18n.seedFromJsonFiles({ baseDir, locales: ['en','fr'], seedVersion })` on bootstrap.
